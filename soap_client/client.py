    ...
    try:
        ...
        if event == "Створити базу даних":
            ...
            if event == "Додати":
                ...
                soap_client.service.add_column(values["-TABLE-NAME-"], new_column)
                LOCAL_DATA["selected_table"] = values["-TABLE-NAME-"]

        elif event == "Вставити рядок":
            table = soap_client.service.get_table(LOCAL_DATA["selected_table"])
            ...
            if event == "Вставити":
                ...
                soap_client.service.add_row(table_name=table.name, data=values)

        elif event == "Вилучити рядок":
            event, values = delete_row_view()
            if event == "Вилучити":
                soap_client.service.delete_row(
            ...
            if event == "Змінити":
                ...
                if change_event == "Змінити":
                    ...
                    soap_client.service.change_row(
            ...
            if event == "Сполучити":
                view = soap_client.service.join_tables(
            ...
            if event == "Зберегти":
                soap_client.service.save_database(values["-DB-SAVE-PATH-"])
            ...
            if event == "Відкрити":
                soap_client.service.open_database(values["-DB-OPEN-PATH-"])
                sg.popup("Базу даних успішно завантажено!")
                LOCAL_DATA["selected_table"] = None

        if should_re_render:
            window["-CURRENT-DB-"].update(soap_client.service.get_database_name())
            window["-TABLE-LIST-"].update(list(soap_client.service.get_database_tables()))
